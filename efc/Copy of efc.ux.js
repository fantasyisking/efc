...
efc.ux.imageWindow = Ext.extend(efc.ux.Window, {
		...
		var config = {
			items : [{
		        ...
		        html :'<div id="mapPic"><div class="nav">'  
		            ...
		            +'<img id="' + this.viewImageId + '" src="" border="0" style="cursor: url(/teds/resource/image/openhand_8_8.cur), default;" > </div>'  
		    }]
		}
	...
	pageInit : function (){   
	    ...
	    if(image!=null){   
	        ...
	        Ext.getDom(this.viewImageId).onmousewheel = function ue_mouesewheel(e){
				if (e.wheelDelta){
					if (e.wheelDelta > 0){
	...
	setSizeByImage : function(aglinType){
	    ...
	    if (Ext.isEmpty(scope.imageFactWidth)) scope.imageFactWidth = 1024;
	...
	setImage : function(url){
		Ext.get(this.viewImageId).dom.src= url;
		this.pageInit();
	}
});
