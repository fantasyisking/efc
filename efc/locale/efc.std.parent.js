    	...
    	if (parent.Ext.isEmpty(opts.retrieveType) || opts.retrieveType == '1'){
			...
			this.form.load({
						...
						success : function(form, action) {// 加载成功的处理函数
							...
							if (msg == 'ok') {
								if (!parent.Ext.isEmpty(options.retrieveEnd)) {
									options.retrieveEnd();
								}
							}
...
efc.std.parent.Window = Ext.extend(parent.Ext.Window, {
	...
	buttonsType : 0,	// 1 ： 保存 关闭	2 : 保存 删除 关闭   3 ：关闭
	buttons : [],
...
efc.std.parent.RSComboBox = Ext.extend(parent.Ext.form.ComboBox, {
	...
	initComponent:function() {
    	...
    	var comboStore = new parent.Ext.data.Store({
				...
				reader : new parent.Ext.data.JsonReader({}, this.Records)
			});
		comboStore.load();
...
// 图像浏览
efc.ux.imageWindow = Ext.extend(efc.std.parent.Window, {
	width : 700,   
    height : 542,
		...
		var config = {
			items : [{   
		        ...
		        html :'<div id="mapPic"><div class="nav">'  
		            ...
		            +'<img id="view-image" src="" border="0" style="cursor: url(/tvds/resource/image/openhand_8_8.cur), default;" > </div>'  
		    }]
		}
	...
	//
	pageInit : function (){   
	    var image = parent.Ext.get('view-image');   
	  	var scope = this;
	  	
	    if(image!=null){   
	        parent.Ext.get('view-image').on({   
	        ...
	        parent.Ext.getDom('view-image').onmousewheel = function ue_mouesewheel(e){
				if (e.wheelDelta){
					if (e.wheelDelta > 0){
	...
	setImage : function(url){
		parent.Ext.get('view-image').dom.src= url;
	}
});
